...
fn resolve_algorithms(
  ...
  let override_map: Option<HashMap<String, PathBuf>> =
    if let Some(json_str) = &args.component_overrides {
      Some(serde_json::from_str(json_str).map_err(ConfigError::ParseAlgoOverrideJson)?)
    } else {
      None
    };

  let mut resolved_commands: AlgorithmCommandMap = HashMap::new();
  ...
  for task in tasks {
    ...
    let base_command = if let Some(map) = &override_map {
      ...
      if let Some(path) = map.get(executor) {
        tracing::debug!(
...
impl TryFrom<RunArgs> for Config {
  ...
  fn try_from(args: RunArgs) -> Result<Self, Self::Error> {
    ...
    // Parse Tasks
    let tasks: Tasks =
      serde_json::from_str(&args.tasks).map_err(ConfigError::ParseAlgorithmsJson)?;
    ...
    let algorithm_commands = resolve_algorithms(&args, &tasks, &manifest)?;
