...
### Executor Executable
  ...
  * **Must** accept the `target` name as the first positional argument.
  * **May** accept optional target-specific arguments as `--key=value` flags.
  ...
  * **Must** print results to `stdout` in a simple CSV format: `data_id,duration_nanos`. The `data_id` *must* match the one received from the generator.
  * `stderr` will be captured and forwarded by `impa` for logging.

**Example Output (from the Zig executor):**
(This output corresponds to the generator input above for the target `linear_search`)
...
impa run \
    ...
    --tasks '[{"executor": "zig-executors", "target": "linear_search"}, {"executor": "zig-executors", "target": "binary_search"}, {"executor": "python-executors", "target": "linear_search_py"}]' \
    -- \
    --size 10000
```

  * `--generator "search-ints-deno"`: Use the generator named in its `impafile.toml`.
  * `--tasks '...'`: A JSON array specifying the tasks to run, including the executor name and target.
...
impa run \
    --generator "none" \
    --tasks '[{"executor": "zig-executors", "target": "fib_recursive"}, {"executor": "zig-executors", "target": "fib_iterative"}]'
```
...
`impa` captures the `data_id,duration` CSV output from all tasks and prints it to its own `stdout` as structured, newline-delimited JSON (JSONL). The output includes the `task_hash` (which uniquely identifies a task, i.e., the executor, target, and the args that were part of that payload) and `task_index`, providing full traceability.

```json
{"task_index":0,"task_hash":"22e212d55ee1c69b","executor":"zig-executors","target":"linear_search","args":{},"data_id":"run_1","duration":450}
{"task_index":1,"task_hash":"f11781d8a628a937","executor":"zig-executors","target":"binary_search","args":{},"data_id":"run_1","duration":30}
{"task_index":2,"task_hash":"5638cbb2d40e0b36","executor":"python-executors","target":"linear_search_py","args":{},"data_id":"run_1","duration":52000}
{"task_index":0,"task_hash":"22e212d55ee1c69b","executor":"zig-executors","target":"linear_search","args":{},"data_id":"run_2","duration":455}
{"task_index":1,"task_hash":"f11781d8a628a937","executor":"zig-executors","target":"binary_search","args":{},"data_id":"run_2","duration":31}
{"task_index":2,"task_hash":"5638cbb2d40e0b36","executor":"python-executors","target":"linear_search_py","args":{},"data_id":"run_2","duration":52150}
```
...
**Key Arguments:**
  ...
  * `--tasks <JSON_STRING>`: (Required) A JSON array specifying the tasks to run, including the executor and target.
      * Example: `'[{"executor": "zig-executors", "target": "linear_search"}]'`
